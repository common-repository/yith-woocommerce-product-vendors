	...
	class YITH_Commissions {
		...
		protected function _admin_init() {
			...
			$this->_messages = apply_filters( 'yith_commissions_admin_notice',
				array(
					'error'   => esc_html__( 'Commission status not updated!', 'yith-woocommerce-product-vendors' ),
					'updated' => esc_html__( 'Commission status updated!', 'yith-woocommerce-product-vendors' ),
					'pay-process' => esc_html__( 'Payment successful. In a few minutes you will receive an email with the outcome of the payment and the commission state will be changed accordingly.', 'yith-woocommerce-product-vendors' ),
					'pay-failed'  => esc_html__( 'Payment failed.', 'yith-woocommerce-product-vendors' )
				)
			);
		...
		public function get_status() {
			 ...
			 */
			return array(
				'paid'       => esc_html__( 'Paid', 'yith-woocommerce-product-vendors' ),
				'unpaid'     => esc_html__( 'Unpaid', 'yith-woocommerce-product-vendors' ),
				'pending'    => esc_html__( 'Pending', 'yith-woocommerce-product-vendors' ),
				'refunded'   => esc_html__( 'Refunded', 'yith-woocommerce-product-vendors' ),
				'cancelled'  => esc_html__( 'Cancelled', 'yith-woocommerce-product-vendors' ),
				'processing' => esc_html__( 'Processing', 'yith-woocommerce-product-vendors' ),
			);
		}
		...
		public function admin_notice() {
			if ( ! empty( $_GET['message'] ) && ! empty( $_GET['page'] ) && $this->get_screen() == $_GET['page'] && isset( $this->_messages[ $_GET['message'] ] ) ) {
				$type = $type_message = sanitize_textarea_field( $_GET['message'] );
				if ( in_array( $type_message, array( 'pay-process' ) ) ) {
					$type = 'update-nag';
				}
				...
				$message = in_array( $type, array( 'updated', 'error' ) ) ? sprintf( "<p>%s %s</p>", $this->_messages[ $type_message ], $text ) : sprintf( "%s %s", $this->_messages[ $type_message ], $text );
		...
		public function add_menu_item() {
			...
			if( apply_filters( 'yith_wcmv_show_commission_page', $is_super_user || $vendor->is_valid() && $vendor->has_limited_access() && $vendor->is_owner() ) ) {
				...
				$args = apply_filters( 'yith_wc_product_vendors_commissions_menu_items', array(
						'page_title' => esc_html__( 'Commissions', 'yith-woocommerce-product-vendors' ),
						'menu_title' => esc_html__( 'Commissions', 'yith-woocommerce-product-vendors' ),
		...
		public function change_commission_view_page_title( $admin_title, $title ) {
			if ( isset( $_GET['page'] ) && sanitize_text_field( $_GET['page'] ) == $this->_screen && ! empty( $_GET['view'] ) ) {
				$title = sprintf( esc_html__( 'Commission #%d details', 'yith-woocommerce-product-vendors' ), absint( $_GET['view'] ) );
				$admin_title = sprintf( esc_html__( '%1$s &lsaquo; %2$s &#8212; WordPress' ), $title, get_bloginfo( 'name' ) );
			}
		...
		public function table_update_status() {
			...
			$commission_id = ! empty( $_GET['commission_id'] ) ? yith_wcmv_sanitize_id( $_GET['commission_id'] ) : 0;

			if ( isset( $_GET['view'] ) ) {
				$args['view']  = sanitize_text_field( $_GET['view'] );
				$commission_id = sanitize_text_field( $_GET['view'] );
			}

			if ( ! empty( $commission_id ) && ! empty( $_GET['new_status'] ) ) {
				$commission = YITH_Commission( $commission_id );
				$new_status = sanitize_text_field( $_GET['new_status'] );
				if ( $commission->update_status( $new_status ) ) {
					$args['message'] = 'updated';
					if( apply_filters( 'yith_wcmv_send_commission_email_on_manually_update', false ) ){
						// emails
						WC()->mailer();
						do_action( "yith_vendors_commissions_{$new_status}", $commission );
					}
				}
			}
		...
		public function add_screen_option() {
			if ( 'toplevel_page_' . $this->_screen == get_current_screen()->id ) {
				add_screen_option( 'per_page', array( 'label' => esc_html__( 'Commissions', 'yith-woocommerce-product-vendors' ), 'default' => 20, 'option' => 'edit_commissions_per_page' ) );
			}
		}
		...
		public function commissions_attribute_label( $attribute_label, $meta_key, $product = false ){
			...
			if( $product && 'post.php' == $pagenow && isset( $_GET['post'] ) && $order = wc_get_order( wc_sanitize_order_id( $_GET['post'] ) ) ){
				/**
