	...
	class YITH_Orders {
		...
		public function create_suborder( $parent_order, $vendor_id, $vendor_products, $posted ) {
			...
			$order_data      = apply_filters( 'woocommerce_new_order_data', array(
					...
					'post_title'    => sprintf( esc_html__( 'Order &ndash; %s', 'yith-woocommerce-product-vendors' ), strftime( _x( '%b %d, %Y @ %I:%M %p', 'Order date parsed by strftime', 'yith-woocommerce-product-vendors' ) ) ),
					...
					foreach ( WC()->shipping->get_packages() as $package_key => $package ) {
						if ( ! empty( $package['yith-vendor'] ) && $package['yith-vendor'] instanceof YITH_Vendor && $package['yith-vendor']->id == $vendor_id ) {
							if ( isset( $package['rates'][ $wc_checkout->shipping_methods[ $package_key ] ] ) ) {
								...
								if ( ! $shipping_item_id ) {
									throw new Exception( sprintf( esc_html__( 'Error %d: Unable to create order. Please try again.', 'yith-woocommerce-product-vendors' ), 527 ) );
								}
							}
						}
					}
		...
		public function suborder_meta_synchronization( $parent_order_id, $parent_order ) {
			...
			if ( ! empty( $suborder_ids ) ) {
				foreach ( $suborder_ids as $suborder_id ) {
					...
					/**
					 * This $_post variable's data are used at line 730.
					 * All data has been validated and escaped inside the
					 * `wc_save_order_items()` function.
					 */
					$_post                  = array(); // phpcs:ignoreLine
					...
					foreach ( $child_items as $child_items_id ) {
							...
							switch ( $meta_key ) {
								...
								case 'refund_line_tax':
									...
									if ( isset( $_post[ $meta_key ][ $parent_item_id ] ) ) {
										$_post[ $meta_key ][ $child_items_id ] = yith_wcmv_sanitize_id( $_post[ $meta_key ][ $parent_item_id ] );
						...
						if ( isset( $_post['line_total'][ $child_items_id ] ) ) {
							$suborder_line_total += wc_format_decimal( yith_wcmv_sanitize_id( $_post['line_total'][ $child_items_id ] ) );
						}
					}
					 ...
					 * $_post data has been validated and escaped inside `wc_save_order_items()` function.
					 *
		...
		public static function save_order_items() {
			...
			if ( isset( $_POST['order_id'] ) && isset( $_POST['items'] ) ) {
				$parent_order_id = wc_sanitize_order_id( $_POST['order_id'] );
				//Check if order have sub-order
				if ( ! wp_get_post_parent_id( $parent_order_id ) ) {
					global $wpdb;
					// Parse the jQuery serialized items
					$_post = $_POST; // phpcs:ignoreLine
							...
							foreach ( $_post['items'] as $meta_key => $meta_value ) {
								if ( ! in_array( $meta_key, array(
									...
									if( isset( $_post['items'][ $meta_key ][ $parent_item_id ] ) ){
										$_post['items'][ $meta_key ][ $child_item_id ] = yith_wcmv_sanitize_id( $_post['items'][ $meta_key ][ $parent_item_id ] );
		...
		public static function remove_order_item() {
			...
			/**
			 * $order_item_ids is sanitized at line 1016 after check if is an array or not.
			 */
			$order_item_ids = $_POST['order_item_ids']; //phpcs:ignoreLine
			if ( ! is_array( $order_item_ids ) && is_numeric( $order_item_ids ) ) {
				$order_item_ids = array( $order_item_ids );
			}
			$order_item_ids = array_map( 'yith_wcmv_sanitize_id', $order_item_ids );
		...
		public static function add_order_note() {
			...
			$post_id   = wc_sanitize_order_id( $_POST['post_id'] );
			...
			if ( $post_id > 0 ) {
				if ( ! wp_get_post_parent_id( $post_id ) ) {
					...
					echo wpautop( wptexturize( esc_html( $note ) ) );
					echo '</div><p class="meta"><a href="#" class="delete_note">' . esc_attr__( 'Delete note', 'yith-woocommerce-product-vendors' ) . '</a></p>';
					echo '</li>';
		...
		public static function order_item_stock( $ajax_call_type ) {
			...
			if ( ! wp_get_post_parent_id( $order_id ) ) {
				$order_item_ids = isset( $_POST['order_item_ids'] ) ? sanitize_text_field( $_POST['order_item_ids'] ) : array();
				$order_item_qty = isset( $_POST['order_item_qty'] ) ? sanitize_text_field( $_POST['order_item_qty'] ) : array();
					...
					foreach ( $order_items as $item_id => $order_item ) {
						...
						if ( $vendor->is_valid() && $_product->exists() && $_product->managing_stock() && isset( $order_item_qty[ $item_id ] ) && $order_item_qty[ $item_id ] > 0 ) {
							...
							if ( 'reduce' == $ajax_call_type ) {
								...
								$note         = sprintf( esc_html__( 'Item #%s stock reduced from %s to %s.', 'yith-woocommerce-product-vendors' ), $order_item['product_id'], $old_stock, $new_stock );
							} elseif ( 'increase' == $ajax_call_type ) {
								$stock_change = apply_filters( 'woocommerce_restore_order_stock_quantity', $order_item_qty[ $item_id ], $item_id );
								$new_stock    = $old_stock + $stock_change;
								$note         = sprintf( esc_html__( 'Item #%s stock increased from %s to %s.', 'yith-woocommerce-product-vendors' ), $order_item['product_id'], $old_stock, $new_stock );
							}
		...
		public static function remove_order_item_meta() {
			...
			$parent_meta_id  = wc_sanitize_order_id( $_POST['meta_id'] );
		...
		public static function add_order_item() {
			...
			if ( ! empty( $_POST['item_to_add'] ) ) {
				...
				if ( is_array( $item_to_add ) ) {
					$item_to_add = array_shift( $item_to_add );
				} else {
		...
		public static function add_line_taxes( $order_id ) {
			...
			$city           = wc_clean( sanitize_text_field( $_POST['city'] ) );
			$order          = wc_get_order( wc_sanitize_order_id( $order_id ) );
			$taxes          = array();
			$shipping_taxes = array();

			// Parse the jQuery serialized items
			$sanitized_items = array_map( 'sanitize_text_field', $_POST['items'] );
			parse_str( $sanitized_items, $items );
		...
		public static function remove_order_tax() {
			...
			if ( ! wp_get_post_parent_id( $order_id ) ) {
				$rate_id              = wc_sanitize_order_id( $_POST['rate_id'] );
		...
		public function woocommerce_email_enabled_new_order( $enabled, $object ) {
			$post_id            = ! empty( $_REQUEST['post_ID'] ) ? yith_wcmv_sanitize_id( $_REQUEST['post_ID'] ) : 0;
			$is_editpost_action = ! empty( $_REQUEST['action'] ) && in_array( $_REQUEST['action'], array(
					'editpost',
					'edit'
				) );

			if ( $is_editpost_action && $post_id && wp_get_post_parent_id( $post_id ) == 0 && $post_id != yit_get_prop( $object, 'id' ) ) {
				return false;
			}
		...
		public function order_refunded( $order_id, $parent_refund_id ) {
			...
			if ( ! wp_get_post_parent_id( $order_id ) ) {
				...
				$line_item_qtys          = ! empty( $_POST['line_item_qtys'] ) ? array_map( 'sanitize_text_field', json_decode( $_POST['line_item_qtys'], true ) ) : array();
				$line_item_totals        = ! empty( $_POST['line_item_totals'] ) ? array_map( 'sanitize_text_field', json_decode( $_POST['line_item_totals'], true ) ) : array();
				$line_item_tax_totals    = ! empty( $_POST['line_item_tax_totals'] ) ? array_map( 'sanitize_text_field', json_decode( $_POST['line_item_tax_totals'], true ) ) : array();
				$api_refund              = ! empty( $_POST['api_refund'] ) && sanitize_text_field( $_POST['api_refund'] ) === 'true';
				$restock_refunded_items  = ! empty( $_POST['restock_refunded_items'] ) && sanitize_text_field( $_POST['restock_refunded_items'] ) === 'true';
				$order                   = wc_get_order( $order_id );
		...
		public function commissions_attribute_label( $attribute_label, $meta_key, $product = false ) {
			...
			$order_id = ! empty( $_GET['post'] ) ? wc_sanitize_order_id( $_GET['post'] ) : 0;
			$order    = wc_get_order( $order_id );
			$order    = apply_filters( 'yith_wcmv_commissions_attribute_label_order_object', $order );

			$is_edit_order_page = apply_filters( 'yith_wcmv_commissions_attribute_label_is_edit_order_page', ! empty( $order_id ) && 'shop_order' == get_post_type( $order_id ) && 'post.php' == $pagenow );
				...
				foreach ( $line_items as $line_item_id => $line_item ) {
					...
					if ( $check ) {
						...
						$url_attribute_label = sprintf(
							...
							$admin_url,
							'commission-id-label',
							esc_html__( 'commission_id', 'yith-woocommerce-product-vendors' ),
							esc_html__( 'status', 'yith-woocommerce-product-vendors' ),
				...
				case 'suborders':
					...
					foreach ( $suborder_ids as $suborder_id ) {
						...
						printf( '<mark class="%s tips" data-tip="%s">%s</mark> <strong><a href="%s">#%s</a></strong> <small class="single-order yith-wcmv-suborder-owner">%s %s</small><br/>',
							...
							$vendor->is_valid() ? $vendor->name : esc_html__( 'Vendor deleted', 'yith-woocommerce-product-vendors' )
						);
						echo '<li>';
					...
					foreach ( $files as $download_id => $file ) {
						if ( $inserted_id = wc_downloadable_file_permission( $download_id, $product_id, $order ) ) {
							...
							if ( isset( $file['name'] ) ) {
								...
								$file_count = sprintf( esc_html__( 'File %d', 'yith-woocommerce-product-vendors' ), $file_counter );
							}
