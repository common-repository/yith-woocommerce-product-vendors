	...
	class YITH_Vendors_Admin {
		...
		public function register_panel() {
			...
			$admin_tabs = apply_filters( 'yith_vendors_admin_tabs', array(
					'commissions'      => esc_html__( 'Commissions', 'yith-woocommerce-product-vendors' ),
					'vendors'          => YITH_Vendors()->get_plural_label( 'ucfirst' ),
					'accounts-privacy' => esc_html__( 'Accounts & Privacy', 'yith-woocommerce-product-vendors' ),
					'premium'          => esc_html__( 'Premium Version', 'yith-woocommerce-product-vendors' ),
				)
			);
				...
				if ( ! empty( $_REQUEST['bulk_edit'] ) ) {
					...
					if ( isset( $_REQUEST['tax_input'][ $vendor_taxonomy_name ] ) ) {
						if ( $current_vendor->is_super_user() ) {
							...
							$new_vendor  = yith_get_vendor( sanitize_text_field( $vendor_slug ), 'vendor' );
					...
					if ( $current_vendor->has_limited_access() ) {
						...
						$vendor_id = yith_wcmv_sanitize_id( $vendor_id );
		...
		public function disabled_manage_other_vendors_posts() {
			...
			$post_id          = yith_wcmv_sanitize_id( $_GET['post'] );
			$post             = get_post( $post_id );
			$is_seller        = $vendor->is_valid() && $vendor->has_limited_access();

			if ( $post && 'product' == $post->post_type ) {
				$post_id        = yit_wpml_object_id( $post_id, 'product', true, $default_language );
				...
				if ( $is_seller && false !== $product_vendor && $vendor->id != $product_vendor->id ) {
					wp_die( sprintf( esc_html__( 'You do not have permission to edit this product. %1$sClick here to view and edit your products%2$s.', 'yith-woocommerce-product-vendors' ), '<a href="' . esc_url( 'edit.php?post_type=product' ) . '">', '</a>' ) );
				}
			}
			...
			if ( $post && 'shop_order' == $post->post_type ) {
				...
				if ( $is_seller && ! in_array( $post->ID, $vendor_orders ) ) {
					wp_die( sprintf( esc_html__( 'You do not have permission to edit this order. %1$sClick here to view and edit your orders%2$s.', 'yith-woocommerce-product-vendors' ), '<a href="' . esc_url( 'edit.php?post_type=shop_order' ) . '">', '</a>' ) );
				}
			}
		}
		...
		public function add_taxonomy_fields( $taxonomy ) {
			$args = array(
				...
				'tax_label'       => get_option( 'yith_vat_label', esc_html__( 'VAT/SSN', 'yith-woocommerce-product-vendors' ) ),
		...
		public function check_duplicate_term_name( $term, $taxonomy ) {
			...
			if ( 'edit_terms' == current_action() && isset( $_POST['name'] ) ) {
				$duplicate = get_term_by( 'name', sanitize_text_field( $_POST['name'] ), $taxonomy );
				...
				$message   = esc_html__( 'A vendor with this name already exists.', 'yith-woocommerce-product-vendors' );
				$title     = esc_html__( 'Vendor name already exists', 'yith-woocommerce-product-vendors' );
				$back_link = admin_url( 'edit-tag.php' );

				$back_link = esc_url( add_query_arg( $back_link, array(
					'action'    => 'edit',
					'taxonomy'  => sanitize_text_field( $_POST['taxonomy'] ),
					'tag_ID'    => yith_wcmv_sanitize_id( $_POST['tag_ID'] ),
					'post_type' => 'product'
				) ) );
				...
				if ( $duplicate ) {
					$to_return = new WP_Error( 'term_exists', esc_html__( 'A vendor with this name already exists.', 'yith-woocommerce-product-vendors' ), $duplicate );
				}

				if ( ! empty( $_POST['yith_vendor_data']['owner'] ) ) {
					$vendor = yith_get_vendor( yith_wcmv_sanitize_id( $_POST['yith_vendor_data']['owner'] ), 'user' );
					if ( $vendor->is_valid() ) {
						$to_return = new WP_Error( 'owner_exists', esc_html__( "You can't associate more vendor shops with the same shop owner.", 'yith-woocommerce-product-vendors' ), $duplicate );
					}
				}
		...
		public function save_taxonomy_fields( $vendor_id = 0 ) {
			...
			if ( empty( $vendor_id ) && isset( $_POST['update_vendor_id'] ) ) {
				$vendor_id = yith_wcmv_sanitize_id( $_POST['update_vendor_id'] );
			}
			...
			if ( isset( $_POST['tag-name'] ) ) {
				$vendor_name = sanitize_text_field( $_POST['tag-name'] );
			} elseif ( isset( $_POST['name'] ) ) {
				$vendor_name = sanitize_text_field( $_POST['name'] );
			}
		...
		public function json_search_customer_name( $query ) {
			...
			if ( isset( $_GET['action'] ) && 'woocommerce_json_search_customers' == sanitize_text_field( $_GET['action'] ) && isset( $_GET['plugin'] ) && YITH_WPV_SLUG == sanitize_text_field( $_GET['plugin'] ) && defined( 'DOING_AJAX' ) && DOING_AJAX ) {
				global $wpdb;
		...
		public static function single_taxonomy_meta_box( $taxonomy = '', $taxonomy_box = '' ) {
			...
			if ( $vendor->is_valid() ) {
				...
				$original_product_id = SitePress::get_original_element_id_by_trid( yith_wcmv_sanitize_id( $_GET['trid'] ) );
			...
			$args = array(
				...
				'taxonomy'          => YITH_Vendors()->get_taxonomy_name(),
				'show_option_none'  => ! $vendor->is_super_user() ? '' : sprintf( esc_html__( 'No %s' ), strtolower( $taxonomy_label['singular_name'] ) ),
			...
			if ( $vendor->is_valid() && $vendor->has_limited_access() && $vendor->is_user_admin() ) {
			    $vendor_name = is_null( $wpml_vendor ) ? $vendor->name : $wpml_vendor->name;
		...
		public function show_admin_bar_visit_store( $wp_admin_bar ) {
			...
			if ( $vendor->is_valid() && $vendor->has_limited_access() && apply_filters( 'woocommerce_show_admin_bar_visit_store', true ) ) {
				...
				$wp_admin_bar->add_node( array(
					'parent' => 'site-name',
					'id'     => 'view-store',
					'title'  => esc_html__( 'Visit Store', 'yith-woocommerce-product-vendors' ),
		...
		public function manage_caps() {
			$is_vendor_panel = isset( $_GET['page'] ) && 'yith_wpv_panel' == $_GET['page'];
		...
		public function get_sidebar_link() {
			$links = array(
				array(
					'title' => esc_html__( 'Plugin documentation', 'yith-woocommerce-product-vendors' ),
			...
			if ( defined( 'YITH_WPV_FREE_INIT' ) ) {
				$links[] = array(
					'title' => esc_html__( 'Discover the premium version', 'yith-woocommerce-product-vendors' ),
					'url'   => $this->_premium_landing,
				);

				$links[] = array(
					'title' => esc_html__( 'Free Vs Premium', 'yith-woocommerce-product-vendors' ),
					'url'   => 'https://yithemes.com/themes/plugins/yith-woocommerce-multi-vendor/#tab-free_vs_premium_tab',
				);

				$links[] = array(
					'title' => esc_html__( 'Premium live demo', 'yith-woocommerce-product-vendors' ),
					'url'   => $this->_premium_live
				);

				$links[] = array(
					'title' => esc_html__( 'WordPress support forum', 'yith-woocommerce-product-vendors' ),
					'url'   => 'https://wordpress.org/support/plugin/yith-woocommerce-product-vendors',
				);

				$links[] = array(
					'title' => esc_html__( 'Changelog', 'yith-woocommerce-product-vendors' ),
			...
			if ( defined( 'YITH_WPV_PREMIUM' ) ) {
				$links[] = array(
					'title' => esc_html__( 'Support platform', 'yith-woocommerce-product-vendors' ),
					'url'   => 'https://yithemes.com/my-account/support/dashboard/',
				);

				$links[] = array(
					'title' => sprintf( '%s (%s %s)', esc_html__( 'Changelog', 'yith-woocommerce-product-vendors' ), esc_html__( 'current version', 'yith-woocommerce-product-vendors' ), YITH_WPV_VERSION ),
					'url'   => 'http://yithemes.com/docs-plugins/yith-woocommerce-multi-vendor/15-changelog-premium.html',
				);

				$links[] = array(
					'title' => esc_html__( 'PayPal sandbox test accounts', 'yith-woocommerce-product-vendors' ),
					'url'   => 'https://developer.paypal.com/developer/accounts/',
				);

				$links[] = array(
					'title' => esc_html__( 'Enable PayPal MassPay', 'yith-woocommerce-product-vendors' ),
		...
		public function add_woocommerce_admin_notice() {
			...
			if ( isset( $_GET['message'] ) ) {
				switch ( sanitize_text_field( $_GET['message'] ) ) {
					case 'success':
						$message = esc_html__( 'Option Saved', 'yith-woocommerce-product-vendors' );
						$type    = 'updated';
						break;
					...
					case 'owner_exist':
						$message = esc_html__( "You can't associate more vendor shops with the same shop owner.", 'yith-woocommerce-product-vendors' );
						$type    = 'error';
						break;
				}
			}
		...
		public function render_products_category_filter() {
			...
			$args = array(
				...
				'show_option_none'   => sprintf( esc_html__( 'No %s' ), strtolower( $taxonomy_label['singular_name'] ) ),
				'option_none_value'  => '',
				'value_field'        => 'slug',
