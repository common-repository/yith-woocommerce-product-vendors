			...
			class YITH_Vendors_Privacy extends YITH_Privacy_Plugin_Abstract {
				...
				public function register_exporter( $exporters ) {
					if( apply_filters( 'yith_wcmv_export_vendor_data', true ) ){
						...
						$exporters['yith_wcmv_vendor_details'] = array(
							'exporter_friendly_name' => esc_html__( 'Vendor Data', 'yith-woocommerce-product-vendors' ),
					...
					if( 'yes' == get_option( 'yith_vendor_exports_commissions', 'yes' ) ){
						...
						$exporters['yith_wcmv_vendor_commissions_data'] = array(
							'exporter_friendly_name' => esc_html__( 'Vendor Commissions Data', 'yith-woocommerce-product-vendors' ),
				...
				public function register_eraser( $erasers ) {
					...
					if ( 'yes' == get_option( 'yith_vendor_remove_user_id_in_commissions', 'no' ) ) {
						...
						$erasers['yith_wcmv_vendor_commissions_data'] = array(
							'eraser_friendly_name' => esc_html__( 'Vendor Commissions Data', 'yith-woocommerce-product-vendors' ),
					...
					if ( 'yes' == get_option( 'yith_vendor_remove_vendor_profile_data', 'no' ) ) {
						...
						$erasers['yith_wcmv_vendor_details'] = array(
							'eraser_friendly_name' => esc_html__( 'Vendor Data', 'yith-woocommerce-product-vendors' ),
					...
					if( $user instanceof WP_User ){
						...
						if( $vendor->is_valid() && $vendor->is_owner( $user_id ) ){
							...
							$data_to_export[] = array(
								...
								'group_label' => esc_html__( 'Vendor Data', 'yith-woocommerce-product-vendors' ),
					...
					if( $user instanceof WP_User ) {
						...
						if ( $vendor->is_valid() && $vendor->is_owner( $user_id ) ) {
							...
							if( 0 < count( $commissions ) ){
								$to_exports = array(
									'id'            => esc_html__( 'Commission ID', 'yith-woocommerce-product-vendors' ),
									'order_id'      => esc_html__( 'Refer to Order ID', 'yith-woocommerce-product-vendors' ),
									'user_id'       => esc_html__( 'User ID', 'yith-woocommerce-product-vendors' ),
									'vendor_id'     => esc_html__( 'Vendor ID', 'yith-woocommerce-product-vendors' ),
									'rate'          => esc_html__( 'Commission rate (%)', 'yith-woocommerce-product-vendors' ),
									'amount'        => esc_html__( 'Commission amount', 'yith-woocommerce-product-vendors' ),
									'status'        => esc_html__( 'Commission status', 'yith-woocommerce-product-vendors' ),
									'type'          => esc_html__( 'Commission Type', 'yith-woocommerce-product-vendors' ),
									'last_edit'     => esc_html__( 'Last Update', 'yith-woocommerce-product-vendors' ),
									'last_edit_gmt' => esc_html__( 'Last Update (GMT)', 'yith-woocommerce-product-vendors' ),
								);
								...
								foreach( $commissions as $commission_id ){
									...
									if( ! empty( $commission ) && $commission instanceof YITH_Commission ){
										...
										$data_to_export[] = array(
											...
											'group_label' => esc_html__( 'Vendor Commissions Data', 'yith-woocommerce-product-vendors' ),
				...
				public function get_vendor_personal_data_fields(){
					return apply_filters( 'yith_wcmv_get_vendor_personal_data_fields', array(
							'id'           => esc_html__( 'Vendor ID', 'yith-woocommerce-product-vendors' ),
							'name'         => esc_html__( 'Store Name', 'yith-woocommerce-product-vendors' ),
							'slug'         => esc_html__( 'Store Slug', 'yith-woocommerce-product-vendors' ),
							'description'  => esc_html__( 'Store Description', 'yith-woocommerce-product-vendors' ),
							'paypal_email' => esc_html__( 'Owner PayPal Email', 'yith-woocommerce-product-vendors' ),
						)
					);
				}
				...
				public function privacy_anonymize_data_filter( $anonymous, $type, $data ){
					...
					if( 'yith_wcmv_taxonomy_name' == $type ){
						$anonymous = sprintf( '[%s %s]', esc_html__( 'deleted vendor', 'yith-woocommerce-product-vendors' ), wc_rand_hash() );
					}
				...
				public function get_privacy_message( $section ) {
					...
					switch( $section ){
						case 'collect_and_store':
							$message =  '<p>' . esc_html__( 'We collect information about you during the registration and checkout process on our store.', 'yith-woocommerce-product-vendors' ) . '</p>' .
							            '<p>' . esc_html__( 'While you visit our site, we’ll track:', 'yith-woocommerce-product-vendors' ) . '</p>' .
							            '<ul>' .
							            '<li>' . esc_html__( 'Vendor information: we will use these data to create a vendor profile that allows them to sell products on this website in exchange of a commission fee on each sale.', 'yith-woocommerce-product-vendors' ) . '</li>' .
							            '<li>' . esc_html__( 'The information required to start a vendor shop are the following: name and store description, email, telephone number, PayPal email, information about commissions and issued payments.', 'yith-woocommerce-product-vendors' ) . '</li>' .
							            '</ul>';
							break;

						case 'has_access':
							$message = '<p>' . esc_html__( 'Members of our team have access to the information you provide us. For example, both Administrators and Shop Managers can access:', 'yith-woocommerce-product-vendors' ) . '</p>' .
							           '<ul>' .
							           '<li>' . esc_html__( 'Vendor information', 'yith-woocommerce-product-vendors' ) .'</li>' .
							           '<li>' . esc_html__( 'Data concerning commissions earned by the vendor','yith-woocommerce-product-vendors'  ) .'</li>' .
							           '<li>' . esc_html__( 'Data about payments','yith-woocommerce-product-vendors'  ) .'</li>' .
							           '</ul>' .
							           '<p>' . esc_html__( 'Our team members have access to this information to help fulfill orders, process refunds and support you.', 'yith-woocommerce-product-vendors' ) . '</p>';
							break;
					}
