...
if( ! function_exists( 'install_premium_woocommerce_admin_notice' ) ) {
    ...
    function install_premium_woocommerce_admin_notice() { ?>
        <div class="error">
            <p><?php esc_html_e( 'YITH WooCommerce Multi Vendor is enabled but not effective. It requires WooCommerce in order to work.', 'yith-woocommerce-product-vendors' ); ?></p>
        </div>
        <?php
    }
}
...
if ( defined( 'YITH_WPV_VERSION' ) ) {
	if( ! function_exists( 'yith_wcmv_install_free_admin_notice' ) ){
		function yith_wcmv_install_free_admin_notice() {
			?>
            <div class="error">
                <p><?php esc_html_e( 'You can\'t activate the free version of YITH WooCommerce Multi Vendor while you are using the premium one.', 'yith-woocommerce-product-vendors' ); ?></p>
            </div>
			<?php
		}
	}
    ...
    define( 'YITH_WPV_VERSION', '3.9.0' );
}

if ( ! defined( 'YITH_WPV_DB_VERSION' ) ) {
    define( 'YITH_WPV_DB_VERSION', '1.1.15' );
}
